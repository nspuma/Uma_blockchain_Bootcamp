...
console.log("\nHash for 'Blockchain is simple': ", Helper.sha256Hash("Uma"));
...
console.log('uma - blockchaing validation');
...
let tx = new Transaction("Satheesh", "Chaitra", 10);
    ...
    let blocks = new Block(myblockchain);
    blocks.add_transaction(tx);
    blocks.finalize();
    //let blockchain = new Blockchain();
    myblockchain.add_block(blocks);

    let newBlock = new Block(myblockchain);
    let newTx = new Transaction("Jeeva", "Satheesh", 2);
    newBlock.add_transaction(newTx);
    newBlock.finalize();
    myblockchain.add_block(newBlock);
